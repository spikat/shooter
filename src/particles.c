...
void        fill_crash_part(particle* p, int x, int y)
{
    double  angle = rand() % 360;
    int     velocity = rand() % MAX_CRASH_PARTS_VEL + 1;
    ...
    if (rand() % 2) p->xvel *= -1;
    if (rand() % 2) p->yvel *= -1;
    p->life = rand() % (CRASH_MAX_LIFE - CRASH_MIN_LIFE) + CRASH_MIN_LIFE;
}

int         manage_crashs_particles(shooter_ctx* ctx)
{
    int         i;
    hit_parts*  h;
    hit_parts*  last;

    for (last = NULL, h = ctx->crashs; h; ) {
        h->frame--;
        if (!(h->frame)) {
            if (last) {
                last->next = h->next;
...
int         add_crash(shooter_ctx* ctx, int x, int y)
{
    ...
    for (i = 0; i < NB_CRASH_PARTS; i++) {
        fill_crash_part(&(h->parts[i]), x, y);
    }
    h->frame = CRASH_FRAMES;
    h->x = x;
    h->y = y;
    h->next = ctx->crashs;
    ctx->crashs = h;
